#Spark Program (4A) for Demonstrating Access Control
#
# Counts occurrences of medical condition codes and Social Media messages per user
# Using dataset with PII fields HASHED 
...
if __name__ == "__main__":
    ...
    spark=SparkSession.builder.master('yarn').appName("Analysis-Job-Hash").getOrCreate()
    #spark=SparkSession.builder.master('local[1]').appName("Analysis-Job-Hash").getOrCreate()

    hashDataSchema = StructType([
        ...
        StructField("SynSocial_UID_hashed", StringType()),
        StructField("name_hashed", StringType()),
        StructField("screen_name_hashed", StringType()),
        StructField("filename_hashed", StringType()),
        StructField("Given_Name_hashed", StringType()),
        StructField("Identifier_hashed", StringType()),
        StructField("Synthea_UID_hashed", StringType()),
        StructField("Family_Name_hashed", StringType())    
    ])

    # The file created during the filter operation with all the patients and medical condition codes
    folderName = "hdfs:///DataSet1/hashedDFData"
    # stored locally for test
    #folderName='/home/anne/spark-prototype/S3Data/DataSet1/hashedDFData'

    analyzeDF = spark.read.format('csv').option('header', True).schema(hashDataSchema).load(folderName)
    ...
    totalUsersDF = analyzeDF.select(countDistinct("SynSocial_UID_hashed").alias("No. Social Media Message Sets"))
    #totalUsersDF.show()
    ...
    totalMedRecordsDF = analyzeDF.select(countDistinct("Synthea_UID_hashed").alias("No. Medical Record Sets"))
    ...
    personCodeDF = analyzeDF.filter(analyzeDF.code.isNotNull()).groupBy("SynSocial_UID_hashed", "code").count()
    ...
    # stored in HDFS
    analysisFolderName = "hdfs:///DataSet1/analysis-hash"
    # stored locally for test
    #analysisFolderName='/home/anne/spark-prototype/S3Data/DataSet1/analysis-hash'
    ...
    analysisStatsDFFolderName = "hdfs:///DataSet1/analysis-hash-stats"
    #analysisStatsDFFolderName = '/home/anne/spark-prototype/S3Data/DataSet1/analysis-hash-stats'
    analysisStatsDF.coalesce(1).write.format('csv').option('header', True).mode('overwrite').save(analysisStatsDFFolderName)


    #analysisFileFolderName = "/home/anne/spark-prototype/S3Data/DataSet1/analysis-hash/analysis-hash-stats.txt"
